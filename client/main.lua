...
RegisterNUICallback("GetNearPlayersPolice", function(data, cb)
    ...
    if not foundPlayers then
        exports.pNotify:SendNotification(
            {
                text = '<strong class="red-text">La personne est trop loin de vous</strong>',
                type = "error",
                timeout = 3000,
                layout = "bottomCenter",
                queue = "inventoryhud"
    ...
    function(data, cb)
        ...
        if foundPlayer then
            ...
            elseif data.item.type == "item_keyhouse" then
                ...
                if data.item.name == "crystal" then
                    ESX.ShowNotification("~r~Vous ne pouvez pas donner cet objet.")
                else
